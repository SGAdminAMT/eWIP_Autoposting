select [job_mst].job, [job_mst].suffix, [job_mst].job_date,[job_mst].ord_type, [job_mst].item, [job_mst].stat,[jobroute_mst].[oper_num]
	...
	where job_mst.job = 'BA00305518'--[job_mst].type='J' and [job_mst].stat<>'C' and [jobroute_mst].complete = 0 and [jobroute_mst].[bflush_type] = 'N' and [jobroute_mst].[cntrl_point] = 1 and ([jobroute_mst].[qty_complete] <> [tbJobTransMain].[_qty_complete] or [jobroute_mst].[qty_scrapped] <> [tbJobTransMain].[_qty_scrapped])
	and jobroute_mst.oper_num in ('750','800')