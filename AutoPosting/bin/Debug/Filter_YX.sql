select [job_mst].job, [job_mst].suffix, [job_mst].job_date,[job_mst].ord_type, [job_mst].item, [job_mst].stat,[jobroute_mst].[oper_num]
    ...
    FROM [YixinLive].[dbo].[job_mst]       
	inner join  [YixinLive].[dbo].[jobroute_mst]
	on [job_mst].job = [jobroute_mst].job and  [job_mst].suffix = [jobroute_mst].suffix
	inner join [YXERPSVR].[eWIP_YX].[dbo].[tbJobTransMain]
	on [job_mst].job = [tbJobTransMain]._job and [jobroute_mst].[oper_num] = [tbJobTransMain]._oper_num
	left join [YixinLive].[dbo].[jrt_sch_mst]
	on  [job_mst].[job] = [jrt_sch_mst].[job] and [jrt_sch_mst].[oper_num] = [jobroute_mst].[oper_num]
	left join (select [jobroute_mst].job,max([jobroute_mst].oper_num) as last_oper From [YixinLive].[dbo].[jobroute_mst] Group By [jobroute_mst].job) as maxtable
	on [job_mst].job = maxtable.job
	left join [YixinLive].[dbo].[item_mst]
	on [job_mst].item = [item_mst].item
	left join [YXERPSVR].[eWIP_YX].[dbo].[tbJobRoute]
	...
	--and ([jobroute_mst].[qty_complete] <> [tbJobTransMain].[_qty_complete] or [jobroute_mst].[qty_scrapped] <> [tbJobTransMain].[_qty_scrapped])
	--and ([jobroute_mst].[qty_complete] <> [tbJobTransMain].[_qty_complete] or [jobroute_mst].[qty_scrapped] <> [tbJobTransMain].[_qty_scrapped] or (([jobroute_mst].run_hrs_t_mch+[jobroute_mst].run_hrs_t_lbr)<>ROUND([tbJobTransMain].a_hrs / 60,3)))
	--and job_mst.job ='YXTL230578'
